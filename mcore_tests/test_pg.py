...
ep_generator = get_expert_rank_generator(ep_mp=ep, ep_tp=tp, pp=pp, world_size=WORLD_SIZE)
attn_generator = get_attention_rank_generator(tp=tp, pp=pp, cp=cp, world_size=WORLD_SIZE)
...
def print_pg(pg: dist.ProcessGroup):
    name = pg.name(); group_name = pg.group_name; ranks = get_pg_ranks(pg)

    print(f"{name=} {group_name=} {ranks=}")
...
print_pg(dp)
print_generator_ranks("dp", attn_generator)